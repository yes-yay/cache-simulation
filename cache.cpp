...
public:
    ...
    int get(int address) {
        ...
        for(int i = 0; i < victimCache.size(); i++) {
            if(victimCache[i].address == address) {
        ...
        for (int i = 0; i < writeBuffer.size(); i++) {
            if (writeBuffer[i].address == address) {
    ...
    void printStats() {
        ...
        // Verify that hits + misses = totalAccesses
...
void printCombinedStats(L1Cache& l1, LRUCache& l2) {
    ...
    cout << "L1 Hit Rate: " << (double)l1.hits / totalCPUAccesses * 100.0 << "%" << endl;
    
    // L2 statistics (L2 accesses come from L1 misses)
    cout << "L2 Cache: " << l2.hits << " hits, " << l2.misses << " misses" << endl;
    if (l2.totalAccesses > 0) {
        cout << "L2 Hit Rate: " << (double)l2.hits / l2.totalAccesses * 100.0 << "%" << endl;
    } else {
        cout << "L2 Hit Rate: 0%" << endl;
    }
    
    // Calculate global hit rate
    int totalHits = l1.hits + l2.hits;
    int totalMisses = l2.misses;
    // Global hit rate = (L1 hits + L2 hits) / Total CPU accesses
    double globalHitRate = (double)totalHits / totalCPUAccesses * 100.0;
    // Global miss rate = L2 misses / Total CPU accesses
    double globalMissRate = (double)totalMisses / totalCPUAccesses * 100.0;
